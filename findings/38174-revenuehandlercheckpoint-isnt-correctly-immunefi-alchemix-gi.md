...
- impact: HIGH
- protocol: [[Alchemix]]
- reporter: jasonxiale (Immunefi)
- source: N/A

## Summary


The bug report is about a vulnerability in a smart contract that could lead to the theft of unclaimed yield. The issue is related to a function called `RevenueHandler.checkpoint` in the `RewardsDistributor.sol` file. The vulnerability occurs when the `tokenConfig.poolAdapter` is set to zero, causing the `epochRevenues` record to be incorrect. This means that some users may be able to claim more tokens than expected, while others may not be able to claim any tokens at all. The report includes details on the code that is causing the issue and provides a proof of concept for testing. It is recommended to fix this vulnerability to prevent any potential theft of unclaimed yield.
