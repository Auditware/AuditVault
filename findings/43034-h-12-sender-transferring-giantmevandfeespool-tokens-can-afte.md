...
- impact: HIGH
- protocol: [[Stakehouse Protocol]]
- reporter: JTJabba (Code4rena)
- source: https://code4rena.com/reports/2022-11-stakehouse

## Summary


This bug report discusses an issue where a user's claimed\[] value in the GiantMevAndFeesPool is not adjusted when they transfer away their tokens. This can lead to an integer overflow and prevent the user from accessing certain functions in the pool. A proof of concept is provided to demonstrate this issue and a temporary fix is included. The recommended mitigation steps include reducing the claimed\[] value when necessary or calculating it on a per share basis. 
