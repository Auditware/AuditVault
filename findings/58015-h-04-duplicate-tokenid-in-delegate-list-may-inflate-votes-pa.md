...
The report discusses a bug found in the Velodrome project that could potentially cause issues when moving delegates. The bug occurs because the same checkpoint is used for the move delegate operation, leading to an increase in the number of checkpoints and duplicate tokens in the latest checkpoint. The report recommends adjusting the code to account for scenarios where the operation is performed within the same block.
