...
auditors:
  - "[[0xdeadbeef]]"
  - "[[IllIllI]]"
report: "https://github.com/sherlock-audit/2023-02-gmx-judging"
genome:
  - "[[unbounded-loop]]"
  - "[[known-pattern]]"
  - "[[permanent]]"
...
---
# H-8: Malicious revert reasons with faked lengths can disrupt order execution

- id: 18634
- impact: HIGH
- protocol: [[GMX]]
- reporter: IllIllI (Sherlock)
- source: N/A

## Summary


This bug report is about malicious revert reasons with faked lengths which can be used to disrupt order execution. This is due to a bug in the `ErrorUtils.getRevertMessage()` code, which uses `abi.decode()` to parse the "string" without checking the string's length against the calldata length, or any sort of maximum. A malicious user can use this attack in many different places, such as preventing themselves from being liquidated or ADLed, by providing a malicious string in the revert reason in their [CallbackUtils.afterOrderExecution()] callback, freezing of orders, bypassing pauses by having resting orders that can never execute until allowed, and preventing orders from being canceled when their normal execution reverts.

A test was conducted to show how this can be used to cause a revert, even when the gas to the callback is limited. It was found that if a malicious callback provides a reason "string" that is only a few bytes long, but sets the length to a very large number, when the decode call is made, it will try to read a string of that provided length, and will eventually run out of gas.

The recommended fix for this issue is to have an upper limit on the length of a string that can be passed back, and manually update the length if the string's stated length is greater than the max. This fix has been implemented in the GitHub commit b0bac262191f4f96edc9606192d2e4abfa043dc3.
