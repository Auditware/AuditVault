...
This bug report highlights a problem with the function `getSharesByMintedRsTAO()` in the RivusTAO contract. When the total amount of tokens minted is 0, the function returns 0 instead of the actual amount. This allows attackers to transfer unlimited tokens to other contracts and potentially extract value from small deposits. The report recommends fixing this issue in the RivusTAO and COMAI contracts.
