...
The report describes a bug in a function called `split` that can be exploited by users to gain an arbitrary amount of voting power and balance. This is because the function does not check the amount being split, which can result in one of the split tokens having a negative value. This can be abused by depositing a small amount of tokens and then calling the `split` function with an arbitrary amount to create a large `locked` data. The suggested solution is to validate the provided amount to prevent it from exceeding the locked amount of the split token.
