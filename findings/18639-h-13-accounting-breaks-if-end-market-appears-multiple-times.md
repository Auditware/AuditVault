...
genome:
  ...
  - "[[oracle-freshness]]"
---
# H-13: Accounting breaks if end market appears multiple times in swap path
...
This bug report is about an issue in the GMX-Synthetics contract, which is a protocol for trading synthetic assets. The issue is that if a swap path goes through a market that also is the final destination market, share accounting breaks. This means that the destination market will have extra tokens that its tracking of its own pool amounts doesn't know about, and subsequent markets in the chain will have fewer tokens than their pool amount accounting believes it does.

An attacker, if they're willing to incur the expense of swap fees, can perform swaps back and forth through the same market multiple times and cause a pool to have zero collateral tokens remaining, meaning LP market tokens are unable to withdraw their funds because token transfers will revert. This also results in an undercollateralization, because the reserves calculation will be wrong.

The bug was found manually and the recommended fix is to set a boolean in the `for`-loop calling `_swap()` and pass that variable into `_swap()`, and check the variable rather than using the current check. The fix was implemented in a pull request on GitHub.
