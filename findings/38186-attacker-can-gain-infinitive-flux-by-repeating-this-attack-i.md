...
- impact: HIGH
- protocol: [[Alchemix]]
- reporter: Minato7namikazi (Immunefi)
- source: N/A

## Summary


This bug report is about a vulnerability found in the Smart Contract code for the Alchemix DAO project. The code can be found on GitHub at the specified link. The vulnerability allows for unauthorized minting of NFTs, which can be exploited by a malicious attacker to gain an infinite amount of FLUX tokens. The vulnerability is caused by a flaw in the "reset" function in the Voter contract, which can be used to accrue FLUX tokens for a specific token ID. The attacker can exploit this by repeating a specific attack scenario, where they have three locks with 100k tokens each. By using the "reset" function and merging tokens, the attacker can accrue FLUX tokens even if the same token IDs were already accrued in the same epoch. The impact of this vulnerability is the unauthorized minting of NFTs, which allows the attacker to gain an infinite amount of FLUX tokens. The report includes a proof of concept code that demonstrates the exploit. 
